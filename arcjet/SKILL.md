...
A single project can use both — e.g. request-based on API routes and guard on agent tool calls. If the project already uses Vercel AI SDK, Vercel Eve, Mastra, LangChain, or the Claude Agent SDK, prefer the versioned Guard wrappers in the language reference over hand-wrapping every tool.
...
- **Framework wrappers** (JS `@arcjet/guard/vercel-ai/v7`, `@arcjet/guard/vercel-eve/v0`, `@arcjet/guard/mastra/v1`, `@arcjet/guard/claude-agent-sdk/v0`; Python `arcjet.guard.langchain`) fail closed by default when Guard is unavailable. Import the versioned path — unversioned aliases do not resolve.
