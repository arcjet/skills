...
When the project already uses an official JS agent framework, open the fundamentals file and load **exactly one** adapter from its table. Do not open sibling adapters. Cloudflare Think is the unpublished exception: load the dedicated skill from that table.
...
**JS adapters:** wrappers fail closed; core `guard()` fails open. Load the Step 3 file (or the unpublished Cloudflare Think skill) for the project's framework. `npm install @arcjet/guard` is enough for every adapter that ships in **1.13.0** — plus `actor` / `inputs` / `validateGuardLabel`. Cloudflare Think is not in 1.13.0; pin `@arcjet/guard` to the SHA in [integrate-arcjet-guard-cloudflare-think](../integrate-arcjet-guard-cloudflare-think/SKILL.md).

**Python:** `guard_action` / `guard_action_sync` is core. Load the dedicated skill for the project's extra (`arcjet[langchain]`, `arcjet[langchain-agents]`, `arcjet[openai-agents]`, `arcjet[claude-agent-sdk]`, `arcjet[claude-managed-agents]`, `arcjet[strands-agents]`, `arcjet[google-adk]`). There is no `arcjet[crewai]` extra. `validate_guard_label` and adapter `actor` / `inputs` are in PyPI `arcjet` **1.2.0**. `arcjet[google-adk]` is not in 1.2.0 — pin to the SHA in [integrate-arcjet-guard-google-adk-py](../integrate-arcjet-guard-google-adk-py/SKILL.md).
...
- **Unpublished JS Guard integration skill**: [integrate-arcjet-guard-cloudflare-think](../integrate-arcjet-guard-cloudflare-think/SKILL.md).
