...
name: arcjet
license: Apache-2.0
description: Add Arcjet security protection to any code path — HTTP route handlers, API endpoints, AI agent tool calls, MCP servers, background jobs, and queue workers. Covers rate limiting, bot detection, email validation, prompt injection detection, sensitive information blocking (including Rampart NER), content moderation, remote Guard policies, capture/flush, and abuse prevention. Works with JavaScript/TypeScript, Python, and Go across Next.js, Express, Fastify, SvelteKit, Remix, Bun, Deno, NestJS, FastAPI, Flask, net/http, Vercel AI SDK, Vercel Eve, Mastra, LangChain, and other non-HTTP contexts. Use this skill when the user wants to add security, rate limiting, bot protection, or abuse prevention to any part of their application — whether they say "protect my API," "rate limit tool calls," "block bots," "secure my endpoint," "add security to my MCP server," "guard this Mastra/Eve/AI SDK agent," or "prevent abuse" without mentioning Arcjet specifically.
metadata:
  author: arcjet
---
...
- **Remote policies** (optional): a Console policy selected by `label` can run alongside SDK rules. Pass a trusted `actor` and typed `inputs` (`policyInput` in JS, `server_input` / `local_input` in Python, `GuardPolicyServer*` / `GuardPolicyLocal*` in Go). Plain values are rejected; construct inputs explicitly. `SERVER` values are sent to Arcjet; `LOCAL` strings stay in process. An empty `rules` list still calls Guard so the remote policy can evaluate. Prefer a stable past-tense action label such as `email.sent` when a remote policy will own that action.
