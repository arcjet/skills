...
- **Framework wrappers** (JS `@arcjet/guard/vercel-ai/v7`, `@arcjet/guard/vercel-eve/v0`, `@arcjet/guard/mastra/v1`, `@arcjet/guard/langgraph/v1`, `@arcjet/guard/langchain/v1`, `@arcjet/guard/claude-agent-sdk/v0`, `@arcjet/guard/openai-agents/v0`, `@arcjet/guard/genkit/v1`; Python `guard_action` / `guard_tool` / `ArcjetMiddleware` / `ArcjetCaptureHandler` / `arcjet.guard.crewai`) fail closed by default when Guard is unavailable. JS: import the versioned path – unversioned aliases do not resolve. Python: pick the helper that matches what you hold – any callable → `guard_action` / `guard_action_sync` (core `arcjet.guard`); a LangChain `BaseTool` you call → `guard_tool` (`arcjet[langchain]`, `langchain-core>=1.2.5,<2`); `create_agent` → `ArcjetMiddleware` + `ToolPolicy` (`arcjet[langchain-agents]`, `langchain>=1.3` / `langgraph>=1.2`); observe a chain → `ArcjetCaptureHandler` (`arcjet[langchain]` – cannot deny); official CrewAI crew / LiteAgent / MCP → `register_arcjet_hooks` + `ToolPolicy` (`arcjet[crewai]`, `crewai>=1.15.3,<2`); a CrewAI `BaseTool` you call yourself → `guard_tool` (`arcjet.guard.crewai`). Importing `arcjet.guard.langchain` does not load LangGraph. Importing `arcjet.guard.crewai` does not load LangChain. JS `createAgent` is not Python LangChain and not LangGraph JS.
...
- **LangChain JS `createAgent`:** import `@arcjet/guard/langchain/v1` (`guardTool` + `guardMiddleware` + `langchainContext` only). JS `createAgent` + `createMiddleware({ wrapToolCall })` – not LangGraph `StateGraph`/`ToolNode`, not Python LangChain (`/guards/langchain/`). Authored tools via `guardTool`; unwrapped / MCP via `guardMiddleware`. `humanInTheLoopMiddleware` is HITL, not a policy gate. Denial is `ArcjetDenialResult` as a `ToolMessage`. Until-published (not in npm 1.10.0). Details in the JS Guard reference; docs https://docs.arcjet.com/guards/langchain-js/.
- **CrewAI:** `arcjet[crewai]` (`crewai>=1.15.3,<2`) / `arcjet.guard.crewai`. Official `crewai` only. Process-wide `register_arcjet_hooks` / `unregister_arcjet_hooks` / `ArcjetCrewAIHooks` on `PRE_TOOL_CALL` (sync; `launch_arcjet_sync`). DENY + fail-closed unavailable raise `HookAborted(reason=..., source="arcjet")`. Agent always sees `Tool execution blocked by hook. Tool: {name}`. `POST_TOOL_CALL` is capture only and never rewrites `tool_result`. Optional `guard_tool` for a standalone `BaseTool` you call yourself (only path that raises Arcjet errors). `ToolPolicy` / `sanitize_tool_name` / `free_text_arguments`. No `guard_crew`. Screen inbound with core `guard` / `guard_sync` before `crew.kickoff`. `human_input` is HITL. Until-published — pinned to arcjet-py `cc7dd092` ([guard-crewai-hooks](https://github.com/arcjet/arcjet-py/compare/main...david/cursor/guard-crewai-hooks)); extra is not on PyPI 0.9.0 or current main. Details in the Python Guard reference; docs https://docs.arcjet.com/guards/crewai/.
...
- **Python capture handlers never block**: `ArcjetCaptureHandler` only records. Policy lives in `guard_action` / `guard_tool` / `ArcjetMiddleware`. CrewAI `POST_TOOL_CALL` is the same – capture only and never rewrites `tool_result`; the gate is `PRE_TOOL_CALL` + `HookAborted(reason=..., source="arcjet")`.
