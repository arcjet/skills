...
- **CrewAI:** `arcjet[crewai]` / `arcjet.guard.crewai`. Official `crewai` only. Gate `@on(PRE_TOOL_CALL)` by raising `HookAborted`. `POST_TOOL_CALL` is capture only. Agent/Task `human_input` is HITL, not a policy gate. Fail closed like [#196](https://github.com/arcjet/arcjet-py/pull/196) (`on_guard_error="deny"`). Until-published (extra is not on PyPI 0.9.0 or current main). Details in the Python Guard reference; docs https://docs.arcjet.com/guards/crewai/.
