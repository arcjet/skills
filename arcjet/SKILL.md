...
- **Claude Agent SDK (Python):** import `arcjet.guard.claude_agent_sdk` (`guard_tool` + `guard_hooks` + `claude_agent_context` only). Official `claude-agent-sdk>=0.2.83,<1` via `arcjet[claude-agent-sdk]` (safe extra — no chromadb). Unwrapped built-ins / MCP: `PreToolUse` deny via `guard_hooks`. Authored `@tool`: JSON-in-content + `is_error: True` — Python does **not** forward `structuredContent` (that is the JS adapter). `can_use_tool` is HITL, not policy. Inbound: `UserPromptSubmit` block via `guard_hooks`. No inbound helper. Correlation is a caller-owned UUID `session_id`; never mint. Until-published — do not pin a git SHA; not in PyPI 0.9.0. Details in the Python Guard reference; docs https://docs.arcjet.com/guards/claude-agent-sdk-py/. This is not the JS adapter (`@arcjet/guard/claude-agent-sdk/v0`, `/guards/claude-agent-sdk/`) and not Vercel AI.
