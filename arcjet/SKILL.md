...
name: arcjet
license: Apache-2.0
description: Add Arcjet security protection to any code path – HTTP route handlers, API endpoints, AI agent tool calls, MCP servers, background jobs, and queue workers. Covers rate limiting, bot detection, email validation, prompt injection detection, sensitive information blocking (including Rampart NER), content moderation, capture/flush, OTLP HTTP ingest, and abuse prevention. Distinguishes site SDK keys (`ajkey_`) from project collector keys (`aj_prj_key_`). Works with JavaScript/TypeScript, Python, and Go across Next.js, Express, Fastify, SvelteKit, Remix, Bun, Deno, NestJS, FastAPI, Flask, net/http, Vercel AI SDK, Vercel Eve, Mastra, LangChain (Python and JS createAgent), LangGraph, CrewAI, OpenAI Agents, Genkit, and other non-HTTP contexts. Use this skill when the user wants to add security, rate limiting, bot protection, or abuse prevention to any part of their application – whether they say "protect my API," "rate limit tool calls," "block bots," "secure my endpoint," "add security to my MCP server," "guard this Mastra/Eve/LangGraph/LangChain createAgent/CrewAI/OpenAI Agents/Genkit/AI SDK agent," "send OTLP to Arcjet," "use a collector key," or "prevent abuse" without mentioning Arcjet specifically.
metadata:
  author: arcjet
---
...
Write the site SDK key to the project's env file as `ARCJET_KEY=ajkey_...`. Match whatever the project already does – filename, `.env.example` companion, `.gitignore` entry. If the project doesn't have a convention yet, default to whatever the framework expects and add the env file to `.gitignore`. Never hardcode the key in source.

Arcjet has two credential kinds ([arcjet#8942](https://github.com/arcjet/arcjet/pull/8942), merge `0b49efff`):

- **Site SDK key** (`ajkey_`) – what `sites get-key` / `get-site-key` returns. This is `ARCJET_KEY` for `protect()` / `guard()`.
- **Project collector key** (`aj_prj_key_`) – ingest-only. Capture and OTLP HTTP accept it. Guard and Decide in production return 404 if you send a collector key.

Capture and OTLP HTTP also accept a site key, so an existing `ARCJET_KEY` keeps working for those ingest paths. Do not put a collector key in `ARCJET_KEY` for protect/guard. OTLP ingest is HTTP only – gRPC OTLP is unsupported.
...
- **`capture()` records what happened** after an action (refund issued, tool completed). It is visibility data, never a security decision – it does not deny and never sets `hasFailedOpen()`. Call `flush()` on shutdown so the last batch is not lost. On serverless, pass a platform `waitUntil` (JS) or flush at the end of the invocation. Capture and OTLP HTTP accept a site SDK key (`ajkey_`) or a project collector key (`aj_prj_key_`); collector keys are ingest-only. On Python `guard_action` / `guard_tool` / `ArcjetMiddleware`, `success` is not "the action ran" – see the Python Guard reference.
...
- **Never hardcode `ARCJET_KEY`** – always use environment variables. `ARCJET_KEY` for `protect()` / `guard()` is a site SDK key (`ajkey_`), not a project collector key (`aj_prj_key_`). A collector key on Guard or Decide 404s in production. Capture and OTLP HTTP accept either key. OTLP is HTTP only – do not configure gRPC OTLP.
