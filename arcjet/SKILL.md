...
- **Python framework wrappers** (`guard_action` / `guard_tool` / `ArcjetMiddleware` / `ArcjetCaptureHandler` / `arcjet.guard.crewai` / `arcjet.guard.strands_agents` / `arcjet.guard.claude_agent_sdk` / `arcjet.guard.openai_agents`) fail closed by default when Guard is unavailable. Pick the helper that matches what you hold – any callable → `guard_action` / `guard_action_sync` (core `arcjet.guard`); a LangChain `BaseTool` you call → `guard_tool` (`arcjet[langchain]`, `langchain-core>=1.2.5,<2`); `create_agent` → `ArcjetMiddleware` + `ToolPolicy` (`arcjet[langchain-agents]`, `langchain>=1.3` / `langgraph>=1.2`); observe a chain → `ArcjetCaptureHandler` (`arcjet[langchain]` – cannot deny); official CrewAI crew / LiteAgent / MCP → `register_arcjet_hooks` + `ToolPolicy`; a CrewAI `BaseTool` you call yourself → `guard_tool` (`arcjet.guard.crewai`); official Python Strands Agents `@tool` / unwrapped tools → `guard_tool` + `guard_hooks` + `strands_agent_context` (`arcjet[strands-agents]`, `strands-agents>=1.11.0,<2`); official Python Claude Agent SDK `@tool` / unwrapped built-ins → `guard_tool` + `guard_hooks` + `claude_agent_context` (`arcjet[claude-agent-sdk]`, `claude-agent-sdk>=0.2.127,<1`); official Python OpenAI Agents `FunctionTool` → `guard_tool` + `openai_agents_context` (`arcjet[openai-agents]`, `openai-agents>=0.19.0,<1`). There is no `arcjet[crewai]` extra (CrewAI pulls `chromadb`, CVE-2026-45829) — install `crewai>=1.15.3,<2` yourself. The Strands extra is safe (no chromadb) — users install `arcjet[strands-agents]`. The Claude extra is safe (no chromadb) — users install `arcjet[claude-agent-sdk]`. Importing `arcjet.guard.langchain` does not load LangGraph. Importing `arcjet.guard.crewai` does not load LangChain. Importing `arcjet.guard.strands_agents` does not load LangChain, CrewAI, or JS `@arcjet/guard/strands-agents/v1`. Importing `arcjet.guard.claude_agent_sdk` does not load LangChain, CrewAI, or the JS `@arcjet/guard/claude-agent-sdk/v0` adapter. Importing `arcjet.guard.openai_agents` does not load LangChain. Python OpenAI Agents is not the JS adapter.
