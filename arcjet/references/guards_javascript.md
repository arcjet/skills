...
> - Strands Agents: arcjet-js `23d5a85d1207ab999e1bb883cc1dca1ddb78a858` ([adapter branch](https://github.com/arcjet/arcjet-js/compare/main...david/cursor/guard-strands-agents-v1-hooks-cancel))
...
Exports: `guardTool`, `guardHooks`, `strandsAgentContext`. There is no unversioned `@arcjet/guard/strands-agents` alias. This is official `@strands-agents/sdk` `Agent` + `invoke()` / `stream()` + authored `tool({ callback })` + Plugin / `addHook`. It is not Python `strands`. Until-published: published `@arcjet/guard@1.10.0` does not export `./strands-agents/v1` (`ERR_PACKAGE_PATH_NOT_EXPORTED`). Pin `@arcjet/guard` to git SHA `23d5a85d1207ab999e1bb883cc1dca1ddb78a858` ([adapter branch](https://github.com/arcjet/arcjet-js/compare/main...david/cursor/guard-strands-agents-v1-hooks-cancel)). Optional peer `@strands-agents/sdk` `>=1.1.0 <2`. The existing `strands-agent` example stays with Runtime. Once it ships, the packaged skill is `node_modules/@arcjet/guard/skills/integrate-arcjet-guard-strands-agents`.
