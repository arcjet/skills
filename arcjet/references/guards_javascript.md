...
| Cloudflare Think v0 | `@arcjet/guard/cloudflare-think/v0` | Until-published (not in npm 1.12.0; pin `@arcjet/guard` to git SHA `ADAPTER_SHA`). Official `@cloudflare/think` `>=0.3.0` `Think` subclass + server-side `execute`. Load [integrate-arcjet-guard-cloudflare-think](../../integrate-arcjet-guard-cloudflare-think/SKILL.md) for wiring, pins, deny envelopes, and HITL traps. Gate is `beforeToolCall` block/substitute. Fail closed. Not HITL/`needsApproval`. Not Vercel AI SDK. No unversioned alias. Do not cite an example app until one exists. |
