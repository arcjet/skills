...
> - Cloudflare Think: load [integrate-arcjet-guard-cloudflare-think](../../integrate-arcjet-guard-cloudflare-think/SKILL.md) (pin `ADAPTER_SHA` until the adapter commit exists)
...
| Cloudflare Think v0 | `@arcjet/guard/cloudflare-think/v0` | Until-published (not in npm 1.11.0; pin `@arcjet/guard` to git SHA `ADAPTER_SHA`). Official `@cloudflare/think` `>=0.3.0` `Think` subclass + server-side `execute`. Load [integrate-arcjet-guard-cloudflare-think](../../integrate-arcjet-guard-cloudflare-think/SKILL.md) for wiring, pins, deny envelopes, and HITL traps. Gate is `beforeToolCall` block/substitute. Fail closed. Not HITL/`needsApproval`. Not Vercel AI SDK (`/guards/vercel-ai/`). No unversioned alias. Do not cite an example app until one exists. |
...
| Cloudflare Think | Default DENY is `{ action: "substitute", output: { arcjetDenied: true, … } }`. Optional `onDeny: "block"` returns `{ action: "block", reason }` | `void` / `{ action: "allow" }` executes the tool. A throw is a raw exception / `onChatError`. `needsApproval` is HITL, not a denial. Not Vercel AI SDK `guardTool`. Load the dedicated skill |
