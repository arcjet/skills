...
> _Runtime support last verified against the published `@arcjet/guard` **v1.12.0** on **September 16, 2026**. That release adds `google-adk/v2`, `tanstack-ai/v0`, and `claude-managed-agents/v0` to the 1.11.0 adapter set (`vercel-ai/v7`, `vercel-eve/v0`, `mastra/v1`, `langgraph/v1`, `langchain/v1`, `claude-agent-sdk/v0`, `openai-agents/v0`, `genkit/v1`, `strands-agents/v1`). Prompt-injection `threshold` / `score` are **removed**. Decide timeout defaults to 2000 ms. `@arcjet/guard/cloudflare-think/v0` is not in 1.12.0 — load [integrate-arcjet-guard-cloudflare-think](../../integrate-arcjet-guard-cloudflare-think/SKILL.md) (pin `863ac0d98070529436a6528a4c80b779c3857328`). Read the installed package's types. Minimums tend to creep upward – check the [Runtime support section](https://github.com/arcjet/arcjet-js/tree/main/arcjet-guard#runtime-support) of the README._
...
| Cloudflare Think v0 | `@arcjet/guard/cloudflare-think/v0` | Until-published (not in npm 1.12.0; pin `@arcjet/guard` to git SHA `863ac0d9`). Official `@cloudflare/think` `>=0.3.0 <1` `Think` subclass + server-side `execute`. `guardHooks` + `cloudflareThinkContext` only — no `guardTool`. Load [integrate-arcjet-guard-cloudflare-think](../../integrate-arcjet-guard-cloudflare-think/SKILL.md) for wiring, pins, deny envelopes, and HITL traps. Gate is `beforeToolCall` block/substitute. Default DENY is substitute; `onDeny: "block"` is real DENY only (unavailable stays substitute). Fail closed. Not HITL/`needsApproval`. Not Vercel AI SDK. No unversioned alias. Example: [`examples/cloudflare-think-agent`](https://github.com/arcjet/examples/tree/main/examples/cloudflare-think-agent) (vendors `863ac0d9`; no verify fixture). |
