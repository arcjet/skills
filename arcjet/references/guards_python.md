...
> _Published PyPI release last verified: `arcjet` **v1.1.0** on **September 16, 2026**. That wheel includes `guard_action`, LangChain (`arcjet[langchain]` / `arcjet[langchain-agents]`), CrewAI (`arcjet.guard.crewai`, no extra), OpenAI Agents (`arcjet[openai-agents]`), Claude Agent SDK (`arcjet[claude-agent-sdk]`), Claude Managed Agents (`arcjet[claude-managed-agents]`), Strands Agents (`arcjet[strands-agents]`), `ModerateContent`, `with_rule()`, `protect_signup()`, required HTTP `mode=`, and typed `server_input` / `local_input`. Claude Agent SDK, Claude Managed Agents, and Strands extras need **1.1.0**. `experimental_ModerateContent` remains a deprecated alias._
>
