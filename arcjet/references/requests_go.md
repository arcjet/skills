...
The published latest tag is **`github.com/arcjet/arcjet-go` v0.1.0** (June 30, 2026). A **v1.0.0-rc.1** pre-release exists (August 27, 2026); `go get ...@latest` still resolves v0.1.0. Nested `Metadata`, `WithIPSrc`, Rampart, `decision.IP.Threat`, and Protect transport-failure ERROR decisions are on the module default branch / that rc. The module declares **Go 1.25** in `go.mod`; if the project uses an older Go toolchain, warn the user and stop until it is upgraded.
