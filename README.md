...
Dedicated Python Guard integration skills cover official LangChain, CrewAI, OpenAI Agents, Claude Agent SDK, Claude Managed Agents, Strands Agents, and Google ADK adapters. A dedicated Go Guard integration skill covers Microsoft Agent Framework for Go. JS Guard adapters that ship on npm are per-file references inside the `arcjet` skill (`references/guards_js_*.md`) — load fundamentals plus exactly one adapter. Unpublished JS Cloudflare Think is a dedicated skill. Shared fundamentals stay in the `arcjet` skill. HTTP frameworks are not separate skills.
