...
Dedicated Python Guard integration skills cover official LangChain, CrewAI, OpenAI Agents, Claude Agent SDK, Claude Managed Agents, Strands Agents, and Google ADK adapters. A dedicated Go Guard integration skill covers Microsoft Agent Framework for Go. JS Guard adapters are per-file references inside the `arcjet` skill (`references/guards_js_*.md`) — load fundamentals plus exactly one adapter. JS Cloudflare Think is a dedicated skill (`npm install` / `pnpm add @arcjet/guard` plus the `@cloudflare/think` peer). Shared fundamentals stay in the `arcjet` skill. HTTP frameworks are not separate skills.
...
| `integrate-arcjet-guard-cloudflare-think` | JS Cloudflare Think (`guardHooks` / `cloudflareThinkContext`, `@cloudflare/think>=0.3.0 <1`) |
